		...
		methods: {
			...
			query(inputFwm) {
				...
				if (inputFwm == '' || inputFwm == null) {
					...
					this.$route.params.language == "cn" ? text = "验证码请勿携带汉字！" : text =
						'Please do not carry Chinese characters with CAPTCHA!';
					this.makeToast("danger", text)
					this.alertboll = false
					this.inputFwm = ''
					return;
				} else if (!(/^[0-9]*$/g.test(inputFwm))) {
					this.$route.params.language == "cn" ? text = "请输入纯数字组合！" : text = 'Please enter a number!';
					this.makeToast("danger", text)
					this.alertboll = false
					this.inputFwm = ''
					return;
